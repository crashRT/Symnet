...
package object instructions {
  ...
  def optionToStatePair(previousState: State, error: ErrorCause)(block: State => Option[MemorySpace]): (List[State], List[State]) = {
    ...
    nm.map(m => (List(State(m, previousState.history, None, previousState.instructionHistory, previousState.locationStates)), Nil)).
      getOrElse((Nil, List(State(previousState.memory, previousState.history, Some(error), previousState.instructionHistory, previousState.locationStates))))
  }
